...
from modules.mmseg.utils.class_names import set_dataset_meta
...
class MMSegInferencer_Local(BaseInferencer):
    ...
    def __init__(self,
        ...
        self.visualizer.dataset_meta = set_dataset_meta(dataset_name)

    def set_dataset_meta(self, dataset_name):
        # 使用反射获取类对象
        dataset_class = globals()[dataset_name]

        # 获取类的 METAINFO 属性
        meta_info = dataset_class.METAINFO

        # 将 METAINFO 转换为指定的字典形式
        result = {
            'classes': list(meta_info['classes']),
            'palette': list(meta_info['palette'])
        }
    ...
    def postprocess(self,
        ...
        for i, pred in enumerate(preds):
            ...
            if pred_out_dir != '':
                ...
                img_name = os.path.splitext(os.path.basename(pred.img_path))[0] + '.png'
