...
class EpochBasedTrainLoop_Flow(BaseLoop):
    ...
    def run_epoch(self) -> None:
        ...
        self._epoch += 1
        if hasattr(self.runner, 'epoch_flow'):
            self.runner.epoch_flow.put(self.epoch)
            if self.runner.epoch_flow.qsize() > 3:
                self.runner.epoch_flow.get()
            else:
                pass
    ...
    def run_iter(self, idx, data_batch: Sequence[dict]) -> None:
        ...
        self.runner.call_hook(
            ...
            outputs=outputs)
        self._iter += 1
        if hasattr(self.runner, 'iter_flow'):
            self.runner.iter_flow.put(self.iter)
            if self.runner.iter_flow.qsize() > 3:
                self.runner.iter_flow.get()
            else:
                pass
...
@LOOPS.register_module()
class IterBasedTrainLoop_Flow(BaseLoop):
    ...
    def run_iter(self, data_batch: Sequence[dict]) -> None:
        ...
        if hasattr(self.runner, 'iter_flow'):
            self.runner.iter_flow.put(self.iter)
            if self.runner.iter_flow.qsize() > 1:
                self.runner.iter_flow.get()
            else:
                pass
...
class ValLoop_Loss(BaseLoop):
    ...
    def run_iter(self, idx, data_batch: Sequence[dict]):
        ...
        with autocast(enabled=self.fp16):
            outputs, losses = self.runner.model.val_step(data_batch)
