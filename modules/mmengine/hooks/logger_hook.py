...
class LoggerHook_Plus(Hook):
        ...
        if self.every_n_inner_iters(batch_idx, self.interval):
            ...
            for key, value in outputs.items():
                if key is not None:
                    ...
                    new_key = key
                tag[new_key] = float(value)
                log_str += f" {new_key}: {float(value)}"
            runner.logger.info(log_str)
            match_det = re.search(r'\[(\d+)\]\[\s*(\d+)/(\d+)\]', log_str)
            match_seg = re.findall(r'\d+', log_str)
            if match_det:
                val_epoch = int(match_det.group(1)) / runner.val_interval
                val_iter = int(match_det.group(2))
                val_itersum = int(match_det.group(3))
                step = (val_epoch - 1) * val_itersum + val_iter
                runner.visualizer.add_scalars(
                    tag, step=step, file_path=self.json_log_path)
            elif match_seg:
                val_epoch = runner.iter / runner.val_interval
                val_iter = int(match_seg[0])
                val_itersum = int(match_seg[1])
                step = (val_epoch - 1) * val_itersum + val_iter
                runner.visualizer.add_scalars(
                    tag, step=step, file_path=self.json_log_path)
    ...
    def after_val_epoch(self,
        ...
        tag_plus = OrderedDict()
        for key, value in metrics.items():
            if isinstance(value, np.ndarray):
                tag_plus[key] = value.tolist()
        ...
        if self.log_metric_by_epoch:
            runner.visualizer.add_scalars(
                tag_plus, step=runner.epoch, file_path=self.json_log_path)
        else:
            runner.visualizer.add_scalars(
                tag_plus, step=runner.iter, file_path=self.json_log_path)
